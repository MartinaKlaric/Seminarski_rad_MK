            ...
            <div class="p-6">
                <form action="{{ route('users.update', $user) }}" method="POST">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ $user->name }}" required class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1">
                    </div>

                    <div>
                        <label for="email">Email</label>
                        <input type="email" name="email" id="email" value="{{ $user->email }}" required class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1">
                    </div>
                    ...
                    <select name="role_id" id="role_id" required class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1">
                        @foreach ($roles as $role)
                            <option value="{{ $role->id }}" {{ $role->id == $user->page_id ? 'selected' : '' }}>{{ $role->name }}</option>
                        @endforeach
                    </select>
